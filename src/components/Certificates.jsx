...
// Monochromatic theme-colored issuer vector glyphs
function IssuerLogo({ issuer }) {
  const normalized = issuer.toLowerCase();

  if (normalized.includes('google')) {
    return (
      <svg className="issuer-svg" viewBox="0 0 24 24" fill="currentColor">
  ...
  // Minimal elegant fallback seal
  return (
    <svg className="issuer-svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75">
    ...
    <section id="certificates" className="section-container">
      <div className="section-header fade-up">
        <div className="section-tag-row">
          ...
          <span className="section-category">Credentials</span>
        </div>
        <h2 className="section-heading">
          Licenses <span className="serif-title">&amp; Certifications</span>
        </h2>
            ...
            {cert.skills && cert.skills.length > 0 && (
              <div className="cert-skills-group">
                {cert.skills.map((skill, sIdx) => (
                  <span key={sIdx} className="monochrome-tag sm">
                    {skill}
                  </span>
                ))}
              </div>
      ...
      {certifications.length > 6 && (
        <div className="cert-actions fade-up">
          ...
          >
            <span>{showAll ? 'Collapse Credentials' : `View All Certifications (${certifications.length})`}</span>
          </button>
        </div>
      )}
    </section>
